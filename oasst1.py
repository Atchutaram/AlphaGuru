"""
OpenAssistant (OASST1) multiprocessing tokenizer and sharder for SFT training.
Extracts prompt-response pairs, tokenizes with GPT-2, and saves as train/val split.
"""
...
# Custom GPT-2 tokenizer with added special tokens
...
def process_and_save(pairs, split):
    print(f"Tokenizing {split} set with multiprocessing...")

    nprocs = max(1, mp.cpu_count() // 2)
    with mp.Pool(nprocs) as pool:
        shard = []
        shard_idx = 0
        for result in tqdm(pool.imap(tokenize_pair, pairs, chunksize=64), total=len(pairs)):
            if result is not None:
                shard.append(result)
                if len(shard) >= SHARD_SIZE:
                    save_shard(shard, shard_idx, split)
                    shard_idx += 1
                    shard = []
        if shard:
            save_shard(shard, shard_idx, split)
